          ...
          @for (post of content.posts(); track post.id) {
            ...
            >
              <div class="flex items-center gap-x-4 text-xs">
                <time [dateTime]="post.date" class="text-zinc-500">{{ post.date }}</time>
                <span
              ...
              <div class="relative mt-3">
                <h2
                  class="text-lg font-semibold leading-6 text-zinc-900 dark:text-white group-hover:text-sky-500 transition-colors"
                >
                  {{ post.title }}
                </h2>
              ...
              <div class="relative mt-8 flex items-center gap-x-4">
                <div
                  class="relative h-10 w-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center font-bold text-zinc-500 dark:text-zinc-400 text-sm"
                  aria-hidden="true"
                >
                  JH
                </div>
...
export class Blog {
  ...
  constructor() {
    this.seo.updateSeo({
      ...
      description:
        'Ideas, análisis técnicos y estrategias sobre desarrollo web, arquitectura moderna e inteligencia artificial aplicada a negocios.',
      keywords: 'blog desarrollo web, angular, automatización ia, saas, performance web',
